import threading,subprocess,base64,random,requests,pyautogui
import numpy as np
import os, time
...
class Auto:
    ...
    def DumpXML(self):
        ...
        # print(name)
        os.system(
            f"adb -s {self.handle} shell uiautomator dump && adb -s {self.handle} pull /sdcard/window_dump.xml {name}.xml")
        return name + ".xml"
...
def get_code_hotmail(mail, pwd):
    code = requests.get(f'https://tools.dongvanfb.com/api/get_code?mail={mail}&pass={pwd}&type=instagram').json()[
        "data"]
    return code
            ...
            while True:
                try:
                    ...
                    if poin > [(0, 0)]:
                        ...
                        print(" \033[1;31m |\033[1;37m[", self.nameLD, "]\033[1;31m Mở Face | Time:",
...
def strew():
    pyautogui.doubleClick('1\\1.png')
    time.sleep(2)
    pyautogui.click('1\\2.png')
    time.sleep(80)
    GetDevices()
